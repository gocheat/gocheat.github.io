...
또 한편으로는 클린코드는 개발자의  마인드셋 또는 프로페셔널을 의미한다고 생각합니다.
...
<TechnicalDebt - 클린코드 작성을 위해 개발자들이 언젠가 해결해야 되는 부채>
...
> 개발을 할때 우리는 변하는 것들에 대한 습득을 위해 많은 시간을 소비한다. 위에 말했듯이, 새로운 언어, 새로운 오픈소스, 새로운 아키텍처 등등 우리는 _(많은 개발자들이)_ 트렌드에 시선을 빼앗겨 소프트웨어의 기본에 대해 잊게 된다.
...
- 패키지, 변수, 함수등의 이름을 작성할때 아래와 같은 질문을 통해 의도가 분명한 이름을 사용하는 것이 중요하다. (왜 존재해야 하는가?, 무슨 작업을 하는가?, 어떻게 사용하는가? )
- 의미를 함축하거나 독자(코드를 읽는 사람)가 사전지식을 가지고 있다고 가정하지 말자.
- 변수의 존재 이유, 기능, 사용법 등이 변수/함수/자료형에 드러나야 한다. 따로 주석이 필요하지 않을 정도로.
```javascript
//bad
func savePoint(u []*User){ for _, j := range u { j.totalPoint += rewardPoint } }
 
 
//good
func saveRewardPointToUsers(users []*User){ for _, user := range users { user.totalPoint += rewardPoint }
```
...
- 개발자에게는 특수한 의미를 가지는 단어(List, Array 등)는 실제  저수준 자료구조를 표현한 의미가 아닌 이상 accountList와 같이 변수명에 붙이지 말자. 고수준 비즈니스 정책에 맞는 복수형 단어를 사용한다.
...
1) 주석은 필요악이다. 코드로 의도를 표현하지 못해, 실패를 만회하기 위해 쓰는 것이다. 주석은 언제나 실패를 의미한다.
2) 주석 없이는 자신을 표현할 방법을 찾지 못해 할 수 없이 주석을 사용한다. 주석으로 설명하려 애쓰는 대신에 그 난장판을 깨끗이 치우는 데 시간을 보내야한다,
3) 그럼에도 주석이 필요한 경우는  코드로 의도를 표현 할 수 없을 때이다.
...
- 반복하지 마라 - 동일한 일을 하는 중복코드 발생 시 반드시 함수로 만들어 줘야한다. ( 대개 시간이라는  핑계로 지키지 못하는 것 중 하나 )
- 함수를 만들때 최대한 작게 만들어라 - 함수는 프로그램의 기본 단위이다. 이러한 함수가 작게 만들어질수록 추상화 및 역할 분리가 된다는것을 의미한다.  
- 한가지만 해라 - 함수는 한가지 일만 해야한다. 이로 인한 이점은 유지보수 시 강력한 재사용성 보장으로 인해 리소스가 줄어든다.
- 플래그 인수를 사용하지 마라 - 플래그 인수를 사용한다는 것은 함수 내에서 여러가지 행위를 한다는 것을 의미한다. 앞단계에서 추상화를 통해 행위에 대한 위임을 해야한다.
- 함수의 인자를 최소한으로 해라. -  함수의 인자가 많아질수록 해당 함수를 사용하는 측에서의 이해하기 힘들어지고 복잡도가 증가한다. ( 규칙을 정해 3개 이상일 경우는 하나의 의미있는 자료구조로 선언하여 인자로 사용하라.)
- 함수의 인자로 NULL 을 허용하거나 NULL 을  리턴시키지 마라 (golang은 nil ) - Null이라는 또다른 예외케이스로 인하여 사용하는 측에서 함수 내에서 불필요한 처리 로직이 들어간다.
...
- 테스트 코드는 실제 코드의 유연성, 유지보수성, 재사용성을 보존하고 강화하기 때문이다. 테스트 코드가 방치되어 망가지면 실제 코드도 망가진다. 테스트 코드를 깨끗하게 유지하자.
